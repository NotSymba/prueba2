...
public class IU {
    ...
    public int leeEntero(String msg, int min, int max) {
        ...
        int toret = 0;
        do {
            toret = leeNum(msg + "[" + min + ", " + max + "]");

            if (toret < min || toret > max) {
                System.err.println("Debes introducir un valor entre " + min + " y " + max);
                esValido = false;
            } else {
                esValido = true;
            }
    ...
    public Collection<String> pedirNombresJugadores() {
        ...
        String nombre;
        int numJugadores = leeEntero("Introduce el numero de jugadores: ", 2, 10);
        juego.setNumJugadores(numJugadores);
